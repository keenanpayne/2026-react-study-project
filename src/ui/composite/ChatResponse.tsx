...
export default function ChatResponse(props: ChatResponseProps) {
  return (
    <>
      ...
      <ChatActions count={MockChatResponse.actionsCount} actionsExpanded={props.actionsExpanded} actionOnClick={props.actionOnClick} />
      ...
      <ol className="list-decimal list-outside pl-6 space-y-3">
        {MockChatResponse.questions.map((q) => (
          <li key={q.id}><strong>{q.label}</strong> {q.text}</li>
        ))}
      </ol>
      ...
      <ol className="space-y-5">
        {MockChatResponse.planSections.map((section) => (
          <li key={section.id}>
            <h3 className="font-semibold">{section.id}. {section.title}</h3>
      ...
      <ChatPlan title={MockChatResponse.plan.title} version={MockChatResponse.plan.version} createdAt={MockChatResponse.plan.createdAt} />
    </>
  );
}
